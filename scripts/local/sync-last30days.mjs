...
function claudeSubprocessEnv() {
  // SHELL/USER/LOGNAME confirmed required by testing the identical
  // pattern in the sibling sync-hiring-signals.mjs — the `claude` CLI
  // reported "Not logged in" without them, even fully authenticated, when
  // spawned with only PATH+HOME set.
  const allow = [
    "PATH",
    "HOME",
    "SHELL",
...
function runClaudeHeadless(prompt) {
  return new Promise((resolve, reject) => {
    ...
    // (2.1.251) while building the sibling sync-hiring-signals.mjs.
    // stdio[0] must be explicitly "ignore" — spawn() defaults stdin to an
    // open, never-written pipe, which made the real `claude` CLI wait
    // ~3s for piped input that was never coming, then exit 1. Confirmed
    // by actually running this class of script, not guessed.
    const child = spawn(
      "claude",
